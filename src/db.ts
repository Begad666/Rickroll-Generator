...
const reset_count = async () => {
	await collection.updateOne({ _id: "TotalRRCount" }, { $set: { count: 0 } });
...
const check = async () => {
	...
	if (!globalRRCount) {
		...
		console.log(
			"Successfully created total rickroll count index. Database is ready."
		);
	} else {
		console.log(
			"DB check completed, everything seems to be in order. Please pass appropriate flags if you want to interact with database."
		);
	}
};
